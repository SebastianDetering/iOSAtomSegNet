    ...
    func npDataTypetoSwiftArrayBI<T>( dtype : String, count : Int = 1) throws -> [T] where T: BinaryInteger {
        do {
            ...
            case "<i8":
                ...
                return outputArray
            case "<f2":
                throw FileReadError.WrongBinaryType
            case "<f4":
                throw FileReadError.WrongBinaryType
            case "<f8":
                throw FileReadError.WrongBinaryType
...
func makeInt<T>(dtype : String, raw : T) -> Int64? where T : BinaryInteger {
    switch dtype {
        ...
        default :
            print("we got nowhere, the file data type string from _dictDataType[ dataTypeString ] = " + dtype + "wasn't present in the switch statement.")
            return nil
    }
